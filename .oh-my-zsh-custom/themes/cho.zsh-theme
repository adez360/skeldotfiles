...
PROMPT='%(!.%B%F{red}.%B%F{green}%n@)%m %F{blue}%(!.%1~.%4~) ${vcs_info_msg_0_}'$'\n''%F{blue}└ %(!.#.$)%k%b%f '

RPROMPT=$'%{\e[1A%}''%F{cyan}$(hostname -I | cut -d" " -f1)%f %F{yellow}%*%f'$'%{\e[1B%}'

add_newline_before_prompt() {
    if [[ -z "$_FIRST_PROMPT_DONE" ]]; then
...
catch_clear_command() {
    if [[ "$1" == clear* || "$1" == reset* || "$2" == clear* ]]; then
        unset _FIRST_PROMPT_DONE
    fi
}
add-zsh-hook preexec catch_clear_command
